...
void initialize() {
	...
	if ((fdescriptor= open("/dev/i2c-1", O_RDWR)) < 0)   // check for that
       {
...
void BaroMeter(void){
	...
	pressure=calcPress(bmp085_ReadUP());	
	pressZ = movAvgIntZ(pressure);
	pressure=pressZ;
	usleep(10000);
...
void telemetry_update(void){ 
		...
		volt2 /= 1024.0;   
		volt2 *=2.96;
        sprintf (telemetry_buf, "%0.1f, %0.1f, %d, %d, %d, %d, %d, %0.0f, %0.1f, %0.1f, %.7f, %.7f, %d, %d,  %d, %d, %d, %d,\n", 
        temperature,
        alt, 
        Ptcp.throtler, 
        Ptcp.yaw, 
...
/*
    ...
    pl.servo_3=1;
    pl.servo_4=1;
	std::thread t1 = std::thread(netWork);
